...
#### This app is based on Splunk v2.20.3 and includes the following custom changes:

- Updated scheduled polling cursor handling in `splunk_connector.py`.
- Added `_get_poll_cursor_start_time()` to calculate the next polling cursor from the latest `_indextime`.
- `start_time` is no longer stored directly from the latest `_indextime`.
- Added a configurable asset parameter for scheduled polling cursor lookback in seconds.
- The next polling cursor is stored using a lookback approach: `latest _indextime - poll_cursor_lookback_sec`, rounded down to the nearest minute.
- This change helps reduce missed events caused by indexing delay during scheduled polling.
- Duplicate polling results may increase within the overlap window by design.


## Polling Cursor Overlap Window 

The **Polling cursor overlap window (seconds)** parameter controls how far back the app moves the scheduled polling cursor before saving it to the state file.

Instead of storing the latest `_indextime` directly, the app calculates the next cursor as:

`latest _indextime - polling cursor overlap window`

The result is then rounded down to the nearest minute before being stored as `start_time`.

#### Example: 

- Latest `_indextime`: `10:49:23` 
- Polling cursor overlap window: `180` seconds 
- Calculated time: `10:46:23` 
- Stored cursor: `10:46:00`

This behavior helps reduce missed late-indexed events during scheduled polling. However, because the next poll re-reads part of the previous time window, duplicate results may increase within the overlap window.
...
- For Non-NRI Instance:
  /opt/phantom/local_data/app_states/f2fef467-93b5-455d-b5c9-4afcf0748b38/{asset_id}\_state.json
- For NRI Instance:
  /phantomcyber/local_data/app_states/f2fef467-93b5-455d-b5c9-4afcf0748b38/{asset_id}\_state.json
...
- Helpful examples to run on poll
  ...
  1. The query will execute the query saved in the savedsearch named "Dashboard Views - Action
     History".
  ...
  1. The query will perform statistics for datamodel and will give total count of events fetched
  ...
  1. The query will display field "a" in table format for the results fetched from 'search index
     = "\_internal"' search.
     - on_poll_command: None
     - on_poll_query: index = "\_internal" | table a
     - Final query generated internally: search index = "\_internal" | table a
  1. This query will fetch all the events with sourcetype = "modular_alerts:notable",
  ...
  1. This query will get the count of the events that are indexed in index named "main".
     - on_poll_command: None
     - on_poll_query: index="main" | stats count
     - Final query generated internally: search index="main" | stats count
  1. This query will add a field with name = "a" and value = "abc" in all the events that are
  ...
  1. This query will fetch only the sourcetype of all the events that are indexed in index named
     "main".
     - on_poll_command: None
     - on_poll_query: index="main" | fields sourcetype
     - Final query generated internally: search index="main" | fields sourcetype
  1. This query will fetch all the events having tag = error and index = main.
     - on_poll_command: None
     - on_poll_query: index="\_internal" tag=error
     - Final query generated internally: search index="\_internal" tag="error"
  1. This query will show the data of "ppf_action_history_searches" lookup.
     - on_poll_command: None
